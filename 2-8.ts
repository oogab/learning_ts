// 함수에 타입도 파라미터로 입력 가능
function 마이타입함수<MyType>(x :MyType[]) :MyType {
...
let a = 마이타입함수<number>([4,2])
let b = 마이타입함수<string>(['4', '2'])
...
// 인터페이스를 넣으니까 좀 신기하구만..
function 마이타입함수2<MyType extends LengthCheck>(x :MyType) {
  // 마이타입함수2 입장에서는 만약
